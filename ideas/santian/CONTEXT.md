...
It covers this idea's words only. The thinking that uses them starts at the
[anchor note](./README.md). Arcbyte's own vocabulary — idea, project, artifact,
lens — is in the [root glossary](../../CONTEXT.md).
...
**Santian**:
The product.
_Avoid_: RoutineApp, Timez, Routine. The idea lives at `ideas/santian/` and the
older codebase is called `timez_core`; neither is the product's name, and
"Routine" now means something specific — see below.
...
nothing else; the old project name `RoutineApp` predates it and is unrelated.
